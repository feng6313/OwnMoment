...
struct FrameelevenView: View {
    ...
    @State private var displayHeight: CGFloat = 245
    
    // 显示区域的尺寸常量 - 修改为矩形区域
    private var displayAreaSize: CGSize {
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                ZStack {
                    ...
                    // 在白色背景上层添加矩形图片显示区域，上边缘距离背景上边缘50点，左右居中
                    ZStack {
                        Rectangle()
                            .fill(Color.white) // 暂用白色填充
                            .frame(width: 271, height: 245)
                        ...
                        if let image = selectedImage {
                            ...
                            ZStack(alignment: .bottomTrailing) {
                                Image(uiImage: image)
                                    ...
                                    .frame(width: displayAreaSize.width, height: displayAreaSize.height)
                                    // 应用缩放效果
                    ...
                    .offset(y: -455/2 + 50 + 245/2) // 上边缘距离背景上边缘50点
                    
                    // 添加文字和位置信息，位于白色背景下方336点处，左右居中
                    ...
                    .offset(y: -455/2 + 336) // 位于白色背景下方336点处
                    ...
                    // 添加FunctionSettingsView，替换原有的滑动选择按钮和图标选项栏
...
struct FrameelevenView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        NavigationView {
            FrameelevenView(selectedImage: nil, frameIndex: 0)
        }
    }
}

extension FrameelevenView {
    // 保存图片到相册
        ...
        if #available(iOS 16.0, *) {
            ...
            let renderer = ImageRenderer(content:
                ZStack {
                    ...
                    // 在白色背景上层添加圆形图片显示区域，位于白色背景下方20点处，左右居中
                    ZStack {
                        if let image = selectedImage {
                            ZStack(alignment: .bottomTrailing) {
                                Image(uiImage: image)
                    ...
                    .offset(y: -frameHeight/2 + 50 + displayAreaSize.height/2) // 上边缘距离背景上边缘50点
                    ...
                    .offset(y: -frameHeight/2 + 336) // 位于白色背景下方336点处
...
extension FrameelevenView {
    ...
    // 格式化文本为两行显示：第一行最多15个字符，第二行最多10个字符
